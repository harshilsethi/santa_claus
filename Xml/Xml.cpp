...
void Xml::serialize(Object *toy, std::ostream &os) {
        boost::property_tree::ptree pt;

        Gift gift;
        if(toy) {
                gift.type = toy->getTypeName();
                gift.object = toy->getType();
                gift.title = toy->getTitle();

                pt.add("xml.version", 1);
                ...
                std::cerr << "There is no toy here" << std::endl;
        }
